...
import { Settings, X, UserPlus, CheckCircle2 } from 'lucide-react';
...
export function InvoiceForm({ onSubmit }: InvoiceFormProps) {
  ...
  const handleRegisterOperator = async (e: React.FormEvent) => {
    ...
    if (!newOpName.trim() || !newOpCode.trim()) {
      setRegError('Preencha nome e código');
      return;
    }
    ...
    try {
      ...
      if (insertError) {
        if (insertError.code === '23505') {
          throw new Error('Este código de operador ya existe');
        }
        throw insertError;
      }
      ...
      setRegError(err.message || 'Erro ao cadastrar operador');
    } finally {
      setRegLoading(false);
    }
  };
          ...
          {!showSettings ? (
              ...
              <div className="space-y-6">
                <div className="group">
                  ...
                  <input
                    ...
                    className="w-full px-5 py-4 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-white/20 focus:border-white/20 transition-all font-medium"
            ...
            <div className="p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="text-center">
                <h2 className="text-sm font-black text-white uppercase tracking-[0.3em] flex items-center justify-center gap-2">
                  <UserPlus className="w-4 h-4" /> Cadastrar Operador
                </h2>
              </div>

              <form onSubmit={handleRegisterOperator} className="space-y-6">
                <div className="group">
                  ...
                  <input
                    ...
                    className="w-full px-5 py-4 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 text-white placeholder:text-zinc-700 focus:outline-none focus:ring-2 focus:ring-white/20 transition-all uppercase"
                ...
                <div className="group">
                  <label className="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2">Código do Operador</label>
                  <input
                    type="text"
                    value={newOpCode}
                    onChange={(e) => setNewOpCode(e.target.value)}
                    className="w-full px-5 py-4 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 text-white placeholder:text-zinc-700 focus:outline-none focus:ring-2 focus:ring-white/20 transition-all font-mono"
                    placeholder="EX: 12345"
                    disabled={regLoading || regSuccess}
                  />
                </div>
                ...
                {regError && (
                  <div className="bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-4 rounded-xl text-xs font-bold uppercase tracking-widest text-center">
                    {regError}
                  </div>
                )}

                {regSuccess && (
                  <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-4 py-4 rounded-xl text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2">
                    <CheckCircle2 className="w-4 h-4" /> Operador Cadastrado!
                  </div>
                )}
